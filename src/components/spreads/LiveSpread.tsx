      ...
      <div className="space-y-12 mb-14">
        {LIFE_EXPERIENCES.map((exp, idx) => (
          <section key={exp.id} className="border-t border-[#DDD8CB] pt-8">
            ...
            {/* Content & Artifacts Layout */}
            <div className={`grid grid-cols-1 ${exp.artifacts.length > 1 ? 'lg:grid-cols-12' : 'md:grid-cols-3'} gap-6 mt-4`}>
              {/* Highlights */}
              <div className={`${exp.artifacts.length > 1 ? 'lg:col-span-6' : 'md:col-span-2'} bg-[#FAF8F3] border border-[#DDD8CB] rounded-lg p-5 flex flex-col justify-between`}>
                <div>
              ...
              {/* Photo / Exhibit representation */}
              <div className={`${exp.artifacts.length > 1 ? 'lg:col-span-6 grid grid-cols-1 sm:grid-cols-2 gap-4' : 'md:col-span-1 flex flex-col gap-4'}`}>
                {exp.artifacts.map((art, aIdx) => (
                  <ArtifactExhibit
                    key={art.id}
                    artifact={art}
                    figNum={`${exp.fig} · EXHIBIT ${aIdx + 1}`}
                  />
                ))}
              </div>
            </div>
